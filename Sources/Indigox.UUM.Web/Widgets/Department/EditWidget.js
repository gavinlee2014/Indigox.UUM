define("/UUM/Widgets/Department/EditWidget",
    [
...
function (
    ...
    function exports(widget) {
        ...
        $(widget).Content("EditArea").UserSelect("Manager").first().configure({
            mode: "24",
            multi: false
        });

        $(widget).Content("EditArea").UserSelect("Director").first().configure({
            mode: "24",
            multi: false
        });
        ...
        var organizationalUnitID = Page().getUrlParam("DepartmentID");
        ...
        if (organizationalUnitID) {
            ...
            batch.single({
                ...
                callback: function (data) {
                    ...
                    organizationalUnit.Type = "Department";
        ...
        Page().listenUrlParamChanged(["DepartmentID"], { container: widget }, function () {
            var organizationalUnitID = Page().getUrlParam("DepartmentID");
            var form = $(widget).Content("EditArea").first();
            form.getController().getModel().getProxy().setQuery("OrganizationalUnitQuery");
            form.getController().setParam("OrganizationalUnitID", organizationalUnitID);
