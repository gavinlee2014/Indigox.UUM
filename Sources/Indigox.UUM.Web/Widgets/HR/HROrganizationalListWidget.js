define("/UUM/Widgets/HR/HROrganizationalListWidget",
    [
...
function (
    ...
    var exports = function (widget) {
        ...
        var controller = new ListController({
            model: RecordManager.getInstance().createRecordSet('HROrganizational', {
                proxy: new InstructionProxy({
                    query: "HROrganizationalListQuery"
                })
            }),
            params: {
                "FetchSize": limit
        ...
        var listControl = $(widget).DataList("HROrganizationalList").first();
        ...
        listControl.on("itemAdded", function (source, index, item) {
            ...
            if (!record.get("Synchronized")) {
                $(item).Button("btnSync").on("clicked", function () {
                    var dialog = $.Dialog('HROrganizationalSyncDialog').first();
